...
class AggregateFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Derived features
        agg["FraudRate"] = agg["FraudCount"] / agg["TotalTransactions"]

        agg["PositiveAmountRatio"] = agg["PositiveCount"] / agg["TotalTransactions"]

        # Customer tenure in days
        agg["TenureDays"] = (agg["LastTransaction"] - agg["FirstTransaction"]).dt.days

        # Transaction velocity (transactions per day)
        # Avoid division by zero for single-day customers
        agg["TransactionVelocity"] = agg["TotalTransactions"] / (agg["TenureDays"].replace(0, 1))
        ...
        agg = agg.drop(columns=["FirstTransaction", "LastTransaction"])

        logger.info(
            f"Aggregation complete: " f"{len(agg):,} customers, " f"{agg.shape[1]} features"
        )
        return agg
...
class DropConstantColumns(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        cols_to_drop = [c for c in self.constant_cols_ if c in X.columns]
...
def build_preprocessing_pipeline(
    numerical_cols: list, categorical_cols: list, scaling: str = "standard"
) -> ColumnTransformer:
    """
...
def process_raw_data(input_path: str, output_path: str) -> pd.DataFrame:
    ...
    logger.info(f"Raw data: {df.shape[0]:,} rows, " f"{df.shape[1]} columns")
    ...
    logger.info(
        f"Processed: {df_processed.shape[0]:,} customers, " f"{df_processed.shape[1]} features"
    )
...
def calculate_rfm(df_raw: pd.DataFrame, snapshot_date: str = None) -> pd.DataFrame:
    """
    ...
    last_txn["Recency"] = (snapshot - last_txn["LastTransaction"]).dt.days

    frequency = df.groupby("CustomerId").size().reset_index()
    frequency.columns = ["CustomerId", "Frequency"]

    monetary = df[df["Amount"] > 0].groupby("CustomerId")["Amount"].sum().reset_index()
    monetary.columns = ["CustomerId", "Monetary"]

    rfm = (
        last_txn[["CustomerId", "Recency"]]
...
def assign_risk_labels(
    ...
    scaler = RobustScaler()
    rfm_scaled = scaler.fit_transform(rfm[["Recency", "LogFrequency", "LogMonetary"]])
    ...
    cluster_summary["AvgLogFreq"] = rfm.groupby("Cluster")["LogFrequency"].mean()
    cluster_summary["AvgLogMon"] = rfm.groupby("Cluster")["LogMonetary"].mean()
    ...
    logger.info(f"High-risk cluster identified: " f"Cluster {high_risk_cluster}")
    logger.info(
        f"High-risk profile: "
...
def build_final_dataset(input_path: str, output_path: str) -> pd.DataFrame:
    """
    ...
    df_final = df_features.merge(
        rfm_labeled[["CustomerId", "Recency", "Frequency", "Monetary", "Cluster", "is_high_risk"]],
        on="CustomerId",
        how="left",
    )

    logger.info(
        f"\nFinal dataset: " f"{df_final.shape[0]:,} customers × " f"{df_final.shape[1]} features"
    )
    logger.info(
...
if __name__ == "__main__":
    build_final_dataset(
        input_path="data/raw/data.csv", output_path="data/processed/final_dataset.csv"
    )
